...
bool AirGradient::parse_pm_data(DATA &pm_data)
{
  ...
  uint16_t const frame_length = makeWord(data[2] << 8, data[3]);
  ...
  uint16_t const checksum = makeWord(data[2 + frame_length], data[2 + frame_length + 1]);
  ...
  if (calculated_checksum == checksum) {
    ...
    pm_data.PM_AE_UG_10_0 = makeWord(data[14], data[15]);
